...
const ReservationCreer = () => {
    const [noSalle, setNoSalle] = useState<string>("");
    const {salleList, loading : salleLoading, error : salleError, getSalleByNoSalle} = useSalles();
    ...
    useEffect(() => {
        ...
        salleParNoSalle?.noSalle && getReservationByNoSalle(salleParNoSalle?.noSalle!).then(reservations => setReservations(reservations))
    }, [salleParNoSalle]);
    ...
    const validationSchema = yup.object().shape({
        noSalle: yup.number().required("Choisissez le numéro de la salle"),
    ...
    function handleSalleSelect(noSalle: number) {
        formik.setFieldValue("noSalle", noSalle)
        getSalleByNoSalle(noSalle, true).then(salle => {
            setSalleParNoSalle(salle?.data)
        });
    }
                        ...
                        <Form onSubmit={handleSubmit}>
                            <Form.Group className="mb-3" controlId="noSalle">
                                <Row>
                                    ...
                                    <Col className="col-4">
                                        <Dropdown>
                                            <Dropdown.Toggle variant="secondary" id="dropdown-noSalle">
                                                {values.noSalle || "Choisissez la salle"}
                                            </Dropdown.Toggle>

                                            <Dropdown.Menu>
                                                {salleList && salleList.map((s) =>
                                                    <Dropdown.Item key={s.noSalle} onClick={() => handleSalleSelect(s.noSalle)}>{s.noSalle}</Dropdown.Item>
                                                )}
                                            </Dropdown.Menu>
                                        </Dropdown>
                                        {dirty && <span className={'d-flex px-3 invalid-feedback'}>{errors.noSalle}</span>}
                                    </Col>
                                </Row>
                            </Form.Group>
                            ...
                            <Form.Group className="mb-3" controlId="date">
                                <Row>
                                    ...
                                    <Col className="col-4">
                                        ...
                                        {dirty && <span className={'d-flex px-3 invalid-feedback'}>{errors.date}</span>}
                                    </Col>
                                </Row>
                            </Form.Group>
                            <Form.Group className="mb-3" controlId="reservationTime">
                                <Row>
                                    ...
                                    <Col className="col-4">
                                        ...
                                        {dirty && <span className={'d-flex px-3 invalid-feedback'}>{errors.reservationTime}</span>}
                                    </Col>
                                </Row>
                            </Form.Group>
...
export default ReservationCreer;