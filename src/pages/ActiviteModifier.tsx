...
const ActiviteModifier = () => {
    const {id: nomActivite}= useParams();
    const {loading, error, modifierActivityByNom, activiteParNom} = useActivites(nomActivite);
    const navigate = useNavigate();

    const initialValues = {
        nomActivite: activiteParNom?.nomActivite ?? "",
        description: activiteParNom?.description ?? "",
        estActif: activiteParNom?.estActif ?? true,
    ...
    <>
        <Row className={'gx-0 ps-3'}>
            <Col className={'d-flex justify-content-between'}>
                <h2>Modifier type d’activité</h2>
            </Col>
        </Row>
        <Row className={'gx-0 ps-3'}>
            <Col>
...
export default ActiviteModifier;